...
class VKinderBot:
        ...
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text:
                ...
                request_list = {
                    ...
                    text_emoji('В избранное', 'heart'): {self.add_user_db: [request], self.add_relation: ['Favorite']},
                    text_emoji('Избранное', 'star'): {self.show_like_list: []},
                    text_emoji('Не нравится', 'poop'): {self.add_user_db: [request], self.add_relation: ['Blacklist']},
                }
    ...
    def add_user_db(self, request):
        ...
        if request == 'привет':
            if not self.db.check_user_in_db(self.user['id']).all():
                self.db.add_user([self.user['id'], self.user['first_name'],
                                  self.user['last_name'], sex.get(self.user['sex']),
            ...
            if not self.db.check_user_in_db(self.users_data.get(self.user['id'])).all():
                ...
                self.db.add_user([like_person['id'], like_person['first_name'],
                                  like_person['last_name'], sex.get(like_person['sex']),
    ...
    def next_user(self):
        ...
        try:
            ...
            if self.current_candidate['id'] not in favorite_list and self.current_candidate['id'] not in blocked_list:
    ...
    def __check_mutual_sympathy(self):
        ...
        candidate_favorite_list = [i[0] for i in self.db.get_favorite_list(self.users_data.get(self.user['id']))]
        return self.user['id'] in candidate_favorite_list and self.users_data.get(self.user['id']) in user_favorite_list

    def __get_message_info(self, user):
        name = f'{user[1]} {user[2]}'
        link = f'https://vk.com/id{user[0]}'
        photos = self.vk_interaction.photos_get(user[0])
        attachment = ','.join([f'photo{user[0]}_{ph["id"]}' for idx, ph in enumerate(photos) if idx < 4])
        message = f'{name}\r\n{link}'
        return attachment, message

    def add_relation(self, status):
        self.db.add_relation(self.user['id'], self.users_data.get(self.user['id']), status=status)
        if self.__check_mutual_sympathy():
            attachment, message = self.__get_message_info(self.get_user_info(self.users_data.get(self.user['id'])))
            self.write_msg(f' У вас взаимная симпатия с {message}', attachment=attachment)
        else:
            self.next_user()
